...
- 프론트엔드 UI 2차 개선 완료
- 실행 앱 구조를 `backend`, `frontend` 루트 앱으로 리팩토링 완료
...
| 구분 | 서비스 유형 | 대상 경로 | 역할 |
| --- | --- | --- | --- |
| 백엔드 | Web Service | `backend` | 추천 API 제공 |
| 프론트엔드 | Web Service | `frontend` | Next.js 화면 제공 |
...
Render에서 Next.js 앱을 Web Service로 실행하려면 `frontend/package.json`에 `start` script가 필요하다.
...
백엔드 `ALLOWED_ORIGINS`에는 프론트엔드 Render URL의 origin을 설정한다.
...
- [ ] `frontend/package.json`에 `"start": "next start"` script 존재 여부 확인
- [ ] 백엔드 Render URL 확보
